...
local function clickPunchButton()
    ...
    if punchButton and punchButton.Visible then
        local charges = punchButton:FindFirstChild("Charges")
        if charges and charges.Text ~= "1" then return false end
        pcall(function()
            for _, conn in ipairs(getconnections(punchButton.MouseButton1Click) or {}) do
                conn:Fire()
            end
        end)
        return true
...
local function getNearestKiller()
    ...
    for _, name in ipairs(killerList) do
        ...
        if k and k:FindFirstChild("HumanoidRootPart") then
            ...
            if d < bestDist then bestDist = d best = k.HumanoidRootPart end
        end
    end
    return best
end
...
local function aimAt(targetRoot)
    ...
    local humanoid = char:FindFirstChildOfClass("Humanoid")
    if not hrp or not humanoid or not targetRoot then return end
    humanoid.AutoRotate = false
    local start = tick()
    task.spawn(function()
        while tick() - start < 0.5 do
            if hrp and targetRoot and targetRoot.Parent then
                local pred = targetRoot.Position + targetRoot.Velocity * predictionValue
                hrp.CFrame = CFrame.lookAt(hrp.Position, pred)
            end
            task.wait()
        end
...
local function punchAndAim()
    if tick() - punchCooldown < 0.4 then return end
    local killerRoot = getNearestKiller()
    if not killerRoot then return end
    local myRoot = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not myRoot or (killerRoot.Position - myRoot.Position).Magnitude > 12 then return end
    if clickPunchButton() then
        punchCooldown = tick()
        if aimPunch then aimAt(killerRoot) end
    end
end
...
AutoBlockTab:CreateToggle({Name="Strict Range",CurrentValue=false,Callback=function(v) strictRange=v end})
...
AutoPunchTab:CreateSlider({Name="Aim Prediction",Range={0,5},CurrentValue=predictionValue,Increment=0.1,Callback=function(v) predictionValue=v end})

MiscTab:CreateButton({Name = "Load Fake Block", Callback = function()
    pcall(function()
...
MiscTab:CreateToggle({Name="ESP",CurrentValue=false,Callback=function(v) espEnabled=v end})
