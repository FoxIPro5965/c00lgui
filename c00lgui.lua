...
local function enableFly()
    ...
    flyLoop = RunService.RenderStepped:Connect(function()
        ...
        -- Joystick movement (X Z)
        local moveDir = hum.MoveDirection 
        local horizontal = Vector3.new(moveDir.X, 0, moveDir.Z)

        -- Camera tilt controls up/down (Y)
        local vertical = cam.CFrame.LookVector.Y 

        local flyVec = (horizontal * flySpeed) + Vector3.new(0, vertical * flySpeed, 0)
