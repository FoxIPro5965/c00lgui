--// c00lgui v0.4 + fall mode just for fun//--
...
local function applyInertiaRagdoll()
    ...
    -- Tắt motor & làm ball-socket
    for _, m in ipairs(char:GetDescendants()) do
        if m:IsA("Motor6D") and m.Name ~= "RootJoint" then
            m.Enabled = false
            table.insert(motors, m)

            local att0 = Instance.new("Attachment", m.Part0)
            att0.CFrame = m.C0
            local att1 = Instance.new("Attachment", m.Part1)
            att1.CFrame = m.C1

            local sock = Instance.new("BallSocketConstraint")
            sock.Attachment0 = att0
            sock.Attachment1 = att1
            sock.Parent = m.Part1
            ...
            table.insert(constraints, sock)
        end
    end

    -- Cho va chạm full-body
    ...
    if speed > 2 then
        ...
        -- Xoay player đổ về phía trước/sau/trái/phải
        hrp.CFrame = CFrame.new(hrp.Position) * CFrame.fromMatrix(Vector3.new(), dir, Vector3.new(0,1,0))

        -- Quán tính khi té
        hrp.Velocity = vel * 1.15
    else
        -- Đứng yên → ngã random nhẹ
