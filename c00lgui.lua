...
-- C00Lkidd MODE
--=====================
local KiddButton = Instance.new("TextButton")
KiddButton.Parent = MainFrame
KiddButton.Size = UDim2.new(0,220,0,35)
KiddButton.Position = UDim2.new(0,20,0,285)
KiddButton.BackgroundColor3 = Color3.fromRGB(150,0,0)
KiddButton.TextColor3 = Color3.fromRGB(255,255,255)
KiddButton.Font = Enum.Font.GothamBold
KiddButton.TextSize = 16
KiddButton.Text = "C00Lkidd Mode: OFF"
Instance.new("UICorner", KiddButton).CornerRadius = UDim.new(0,8)

local kiddOn = false
local KiddSound = Instance.new("Sound")
KiddSound.Name = "Spooky"
KiddSound.SoundId = "rbxassetid://104181508980428" -- nhạc C00Lkidd
KiddSound.Volume = 5
KiddSound.Looped = true
KiddSound.Pitch = 0.2
KiddSound.Parent = workspace

local skyID = 158118263 -- id skybox
local function applyKiddSky()
    for _,v in ipairs(Lighting:GetChildren()) do
        if v:IsA("Sky") then v:Destroy() end
    end
...
local function spamDecal(v)
    if v:IsA("Part") then
        for i=0, 5 do
            ...
            D.Face = i
            D.Parent = v
            D.Texture = "http://www.roblox.com/asset/?id="..skyID
        end
...
KiddButton.MouseButton1Click:Connect(function()
    kiddOn = not kiddOn
    if kiddOn then
        KiddButton.Text = "C00Lkidd Mode: ON"
        KiddButton.BackgroundColor3 = Color3.fromRGB(0,180,0)
        applyKiddSky()
        decalspam(skyID)
        KiddSound:Play()
    else
        ...
        for _,v in ipairs(workspace:GetDescendants()) do
            if v:IsA("Decal") and v.Name=="MYDECALHUE" then v:Destroy() end
        end
    end
end)
